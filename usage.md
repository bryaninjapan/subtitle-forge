...
## 🛠️ 2026-04-09 更新日誌 (Local ASR 遷移)

已成功導入本地端音訊轉錄方案，並建立混合架構平衡點：

| 元件 | 狀態 | 理由 |
|:---|:---|:---|
| **ASR** | ✅ **Qwen3-ASR 本地** | 已通過大規模測試，精準度與安全性極佳，節省 100% 雲端費用。 |
| **翻譯** | 維持 DeepSeek-V3 雲端 | 核心品質關鍵，目前本地模型尚未能達到同等水準。 |
| **章節 / QA** | 維持 OpenRouter 雲端 | 避免本地資源同時載入多個巨大模型造成並發衝突。 |
| **筆記 (VLM)** | 維持 Gemini 雲端 | 影片截圖多模態整合目前雲端最為穩定。 |

**架構邏輯優化：**
1. **ASR 本地化**：音訊檔案大、處理耗時長，本地化收益最高。
2. **推理雲端化**：翻譯、章節、QA 屬於純文字推理，雲端 API 成本極低且品質穩定。
